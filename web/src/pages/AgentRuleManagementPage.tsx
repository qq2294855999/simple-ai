...
import {Button, Dropdown, Input, Popconfirm, Select, Space, Spin, Table, Tag, Tooltip, Typography} from "antd";
...
export function AgentRuleManagementPage() {
    ...
    const handleToggleStatus = useCallback(async (id: string) => {
        try {
            const newStatus = await AgentRuleApi.toggleStatus(id);
            ToastUtil.success(`规则已${newStatus === "ON" ? "启用" : "停用"}`);
            loadDataRef.current?.();
        } catch {
    ...
    const columns = useMemo<ColumnsType<AgentRulePageResponseDto>>(() => [
        ...
        {
            title: "定义描述", dataIndex: "definitionDesc", width: 200, ellipsis: true,
            render: (v: string) => <Tooltip title={v}>{v}</Tooltip>
        },
        {
            title: "触发条件", dataIndex: "triggerCondition", width: 200, ellipsis: true,
            render: (v: string) => <Tooltip title={v}>{v}</Tooltip>
        },
        {
            title: "触发动作", dataIndex: "triggerAction", width: 200, ellipsis: true,
            render: (v: string) => <Tooltip title={v}>{v}</Tooltip>
        },
        {
                ...
                return (
                    <Space>
                        <Button type="link" size="small"
                                onClick={() => navigate(`/agent-rule/${record.id}/edit`)}>编辑</Button>
    ...
    return (
        <div>
            ...
            <div className="simple-toolbar">
                ...
                <Button type="primary" onClick={() => navigate("/agent-rule/create")}>新增规则</Button>
            </div>
